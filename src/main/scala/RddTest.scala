...
object RddTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    SparkUtils.printRdd("rdd.map(_ + 1)", rdd.map(_ + 1))

    // 每个元素执行后扁平化，rdd.flatMap(_.to(3)): 1 2 3 2 3 3 3
    SparkUtils.printRdd("rdd.flatMap(_.to(3))", rdd.flatMap(_.to(3)))

    // 条件过滤，rdd.filter(_ < 3): 1 2
    SparkUtils.printRdd("rdd.filter(_ < 3)", rdd.filter(_ < 3))

    // 去重，rdd.distinct(): 1 2 3
    SparkUtils.printRdd("rdd.distinct()", rdd.distinct())

    // 采样，不确定
    SparkUtils.printRdd("rdd.sample(false, 0.5)", rdd.sample(false, 0.5))
    ...
    SparkUtils.printRdd("rdd.union(rdd2)", rdd.union(rdd2))

    // 取相同元素，rdd.intersection(rdd2): 3
    SparkUtils.printRdd("rdd.intersection(rdd2)", rdd.intersection(rdd2))

    // 去除相同元素，rdd.subtract(rdd2): 1 2
    SparkUtils.printRdd("rdd.subtract(rdd2)", rdd.subtract(rdd2))

    // 笛卡尔积，rdd.cartesian(rdd2): (1,3) (1,7) (1,8) (2,3) (2,7) (2,8) (3,3) (3,7) (3,8) (3,3) (3,7) (3,8)
    SparkUtils.printRdd("rdd.cartesian(rdd2)", rdd.cartesian(rdd2))

    val rdd3 = sc.parallelize(List(3, 10, 5, 7, 5))

    SparkUtils.printRdd("rdd3", rdd3)
    ...
    SparkUtils.printArray("rdd3.take(2)", rdd3.take(2))

    // 默认从大到小排序后取前N个元素，rdd3.top(3) descend: 10 7 5
    SparkUtils.printArray("rdd3.top(3) descend", rdd3.top(3))

    // 从小到大排序后取前N个元素，rdd3.top(3) ascend: 3 5 5
    SparkUtils.printArray("rdd3.top(3) ascend", rdd3.top(3)(implicitly[Ordering[Int]].reverse))

    // 随机取N个元素
    SparkUtils.printArray("rdd3.takeSample(false, 2)", rdd3.takeSample(false, 2))
